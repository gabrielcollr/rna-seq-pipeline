...
# Version: 1.0.1
# 
# Description: Script to perform a quality control of raw reads from RNA-Seq sequence data, from NCBI, using FastQC and MultiQC as tools
# 
# Requirements:
...
###### - fastqc -o <output_directory> <input_file>: Performs the quality control of the raw reads and generates an html file with the results
...
# 
# Usage: ./01_script.sh -i <directory_with_raw_reads> -o </path/to/output/directory>
#
# Usage example: ./01_script.sh -i /data/raw_reads -o /home/user/output_directory
#
...
# Define the file with the raw reads as an empty variable for later use with getopts
RAW_READS=""
...
while getopts "i:o:hv" opt; do
 ...
 v) echo -e "\nVersion: $VERSION\n" ;;
 h) echo -e "\nScript to perform a quality control of raw reads from RNA-Seq sequence data, from NCBI, using FastQC and MultiQC as tools"
    echo -e "\nInput options and arguments:"
    echo -e "\t-i: Directory (path) with the RNA-seq raw reads, in fastq format"
    ...
    echo -e "\nUsage: $0 -i <file_with_raw_reads> -o </path/to/output/directory>\n";;
...
# Check if the user has provided the fastq file with the raw reads
...
if [[ "$RAW_READS" == "" ]]; then
 if [[ "$OUTPUT_SELECTED" -ne 1 ]]; then
  ...
  echo -e "\nYou have not provided the fastq file with the raw reads. Use -h for help.\n" >&2
...
for file in "$RAW_READS"/*.fastq.gz ; do
  symb_path=$(readlink -f "$file") # Command not learned to get the absolute path of the original file the link symbolic points to, in case that "file" is a symbolic link.
  if [[ ! -f "$symb_path" ]]; then 
...
} 2> >(tee ./01_logs.err) > >(tee  ./01_logs.out) # Command to redirect standard output and standard input to different files and print them on the screen at the same time.
...
mkdir -p "${OUTPUT_DIRECTORY}"/01_logs/01_logs_general
# Move the logs to the output directory
mv ./01_logs.err "${OUTPUT_DIRECTORY}"/01_logs/01_logs_general
mv ./01_logs.out "${OUTPUT_DIRECTORY}"/01_logs/01_logs_general

# Now we start with the analysis itself

# Create separate directories for the FastQC and MultiQC analyses
FASTQC_OUTPUT="$OUTPUT_DIRECTORY/01_results/fastqc_results"
MULTIQC_OUTPUT="$OUTPUT_DIRECTORY/01_results/multiqc_results"
...
for file in "$RAW_READS"/*.fastq.gz; do
 ...
 FASTQC_LOGS="${OUTPUT_DIRECTORY}"/01_logs/01_fastqc_logs/01_logs_"${SAMPLE_NAME}"
 mkdir -p "${FASTQC_LOGS}"
 {
...
} 2> >(tee "${FASTQC_LOGS}"/01_logs_"${SAMPLE_NAME}".err) > >(tee "${FASTQC_LOGS}"/01_logs_"${SAMPLE_NAME}".out) # Redirect the standard output and standard error to the corresponding log files
done
...
MULTIQC_LOG="${OUTPUT_DIRECTORY}"/01_logs/01_multiqc_logs
mkdir -p "${MULTIQC_LOG}"
{
...
} 2> >(tee "${MULTIQC_LOG}"/01_multiqc_logs.err) > >(tee "${MULTIQC_LOG}"/01_multiqc_logs.out) # Redirect the standard output and standard error to the corresponding log files
